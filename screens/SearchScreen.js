...
import { Text, View, FlatList, StyleSheet, TextInput, TouchableOpacity, Touchable } from 'react-native';
  ...
  fetchMoreTransactions = async () => {
    ...
    if (enterText[0].toUpperCase() === 'B') {
      const query = await db.collection("transactions")
        .where('bookId', '==', text).startAfter(this.state.lastVisibleTransaction)
        .limit(10).get();
      query.docs.map(doc => {
        this.setState({
      ...
      const query = await db.collection("transactions")
        .where('studentId', '==', text).startAfter(this.state.lastVisibleTransaction)
        .limit(10).get();
      query.docs.map(doc => {
        this.setState({
          allTransactions: [...this.state.allTransactions, doc.data()],
...
const styles = StyleSheet.create({
  ...
  searchButton: {
    borderWidth: 1, height: 30, width: 50,
